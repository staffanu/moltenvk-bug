// Compute-shader repro of the MoltenVK / AMD / macOS cache-coherency bug.
// Uses a compute pipeline (B = A ^ 0xA5A5A5A5) to exercise the GPU reading
// host-written data. For the stripped-down version with no shader at all,
// see minimal_repro.cpp (vkCmdCopyBuffer only).
//
// Per Vulkan spec, disjoint nonCoherentAtomSize-aligned ranges of the same
// VkDeviceMemory may be flushed/invalidated concurrently from different threads.
// On the affected stack, the invalidate returns stale data when it races against
// a flush of a disjoint range on another thread.
//
// Build:   c++ -std=c++17 -O2 compute_repro.cpp -lvulkan -o compute_repro
//          (also needs glslc to compile the inline shader; or pre-supply
//          a SPIR-V blob — see SHADER_SPV_PATH).
//
// Run:     VK_LAYER_PATH=~/VulkanSDK/1.4.341.1/macOS/share/vulkan/explicit_layer.d \
//          ./compute_repro
//
